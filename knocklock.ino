...
void send_status(String status, String knock) {
  int tilt_state = is_level() ? 0 : 1;

  Serial.print("STATUS:");
  Serial.print(status);
  Serial.print(",TILT:");
  Serial.print(tilt_state);
  Serial.print(",SOUND:0");
  Serial.print(",KNOCK:");
  Serial.println(knock);
}

void unlock_box() {
  current_status = "ACCESS_GRANTED";
  send_status(current_status, "PATTERN_OK");
  ...
  send_status(current_status, "NONE");
}
...
void report_tilt_if_needed(unsigned long now) {
  if (now - last_tilt_report_time >= tilt_report_interval_ms) {
    ...
    if (!level) {
      ...
      if (current_status != "ACCESS_GRANTED") {
        current_status = "LOCKED";
      }
      send_status(current_status, "NONE");
    }
        ...
        if (gap > restart_gap_ms) {
          ...
          if (interval_count == 3) {
            if (matches_pattern(intervals[0], intervals[1], intervals[2])) {
              ...
              current_status = "ACCESS_DENIED";
              send_status(current_status, "PATTERN_BAD");
              delay(800);
              current_status = "LOCKED";
              send_status(current_status, "NONE");
            }
